...
const resolveMerchantId = (user: User | undefined) => {
  if (!user) return undefined;
  if (user.role === "merchant_admin") return user.id;
  if (user.role === "staff") return user.merchantId;
  return undefined;
};
...
app.post("/cards/link", requireAuth, requireRole(["merchant_admin", "staff"]), (req, res) => {
...
app.post("/topup/initiate", (req, res) => {
  ...
  const topupId = uuid();
  const providerReference = `TOPUP-${topupId.slice(0, 8)}`;
  ...
  recordAudit({ eventType: "topup_initiated", payload: { ...parsed.data, topupId, providerReference } });
  return res.status(202).json({ topupId, status: "pending", providerReference });
});

app.post("/topup/callback", (req, res) => {
  const schema = z.object({
    topupId: z.string(),
    providerStatus: z.enum(["success", "failed"]),
    providerReference: z.string().optional(),
  });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: "Invalid payload" });

  const topup = findTopup(parsed.data.topupId);
  if (!topup) return res.status(404).json({ message: "Top-up not found" });
  if (topup.status !== "pending") return res.status(409).json({ message: "Top-up already finalized", status: topup.status });

  topup.providerReference = parsed.data.providerReference ?? topup.providerReference;
  topup.status = parsed.data.providerStatus === "success" ? "succeeded" : "failed";

  if (topup.status === "succeeded") {
    const central = getCentralWallet();
    const wallet = walletOrCreate(topup.customerId, "customer");
    central.balance += topup.amount;
    wallet.balance += topup.amount;
  }

  recordAudit({ eventType: "topup_callback", payload: { topupId: topup.id, status: topup.status } });
  return res.json({ topupId: topup.id, status: topup.status, providerReference: topup.providerReference });
});

app.get("/topup/status", (req, res) => {
  const schema = z.object({ topupId: z.string() });
...
app.post("/payments/process", requireAuth, requireRole(["staff", "merchant_admin"]), (req, res) => {
  const schema = z.object({ cardUid: z.string(), amount: z.number().positive(), idempotencyKey: z.string() });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: "Invalid payload" });

  const user = (req as any).user as User | undefined;
  const merchantId = resolveMerchantId(user);
  if (!merchantId) return res.status(403).json({ message: "Merchant association missing" });

  const { cardUid, amount, idempotencyKey } = parsed.data;
  ...
  recordAudit({ actorId: user?.id, eventType: "payment_process", payload: { txnId, amount, merchantId } });
